    ...
    while (pos + 8 <= sz) {
        ...
        if (type == 0x0001) {
            ...
            // ── <application> at depth 1 ──────────────────────────────────
                ...
                // The launcher activity is the first one declared in practice,
                // and its orientation is the one that governs the game. Only
                // the first is taken so a settings or splash activity later in
                // the file can't override it.
                for (uint16_t i = 0; i < attrCount; i++) {
                    size_t ap = attrBase + i * attrSize;
                    if (ap + 20 > sz) break;
                    uint32_t an = r32(p + ap + 4);
...
// XAPK manifest.json. Not a general JSON parser.
...
ApkInfo parseApk(const std::string& path) {
    ...
    // ── XAPK? A ZIP of APKs — read manifest.json + icon.png and take arch from
    // its config.<abi> split (a real APK never contains .apk members). ──────
    {
